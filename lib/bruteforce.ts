...
async function sendBruteforceAlertAdvanced(attempt: any): Promise<void> { // eslint-disable-line @typescript-eslint/no-explicit-any
  try {
    const subject = '🚨 Alerte Sécurité - Tentative de Bruteforce Admin Détectée'
    const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; border-radius: 10px;">
        <div style="text-align: center; margin-bottom: 30px;">
          <h1 style="margin: 0; color: #ff6b6b;">🚨 ALERTE SÉCURITÉ ADMIN</h1>
          <p style="margin: 10px 0; font-size: 18px; opacity: 0.9;">Tentative de Bruteforce sur l'Interface Admin</p>
        </div>
        ...
        <div style="background: rgba(255, 255, 255, 0.1); padding: 20px; border-radius: 8px; margin-bottom: 20px;">
          ...
          <ul style="margin: 0; padding-left: 20px;">
            ...
            <li style="margin-bottom: 8px;">✅ Tentatives de connexion admin suspendues</li>
        ...
        <div style="background: rgba(255, 255, 255, 0.1); padding: 20px; border-radius: 8px;">
          ...
          <ul style="margin: 0; padding-left: 20px;">
            ...
            <li style="margin-bottom: 8px;">📊 Analyser les patterns de tentatives admin</li>
        ...
        <div style="text-align: center; margin-top: 30px; padding-top: 20px; border-top: 1px solid rgba(255, 255, 255, 0.2);">
          <p style="margin: 0; opacity: 0.8; font-size: 14px;">
            Cet email a été généré automatiquement par le système de sécurité avancé de TechAnswers Blog.
          </p>
        </div>
      </div>
    `
    ...
    await transporter.sendMail({
      ...
      to: 'admin@techanswers.blog',
      subject,
      html
    })
    
    console.log(`🚨 Alerte bruteforce admin avancée envoyée pour IP: ${attempt.ip}, Fingerprint: ${attempt.fingerprint}, Tentatives: ${attempt.attempts}`)
  } catch (error) {
    console.error('Erreur lors de l\'envoi de l\'alerte bruteforce admin avancée:', error)
  }
} 